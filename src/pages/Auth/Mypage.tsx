...
export default function Mypage() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            if (user) {
                const data: UserData = { 'email': user.email, 'displayName': user.displayName, 'photoURL': user.photoURL,'uid':user.uid }
                setUserData(data);
            } else navigate('/login');
            setLoading(false);
            console.log(user);
    ...
    return (
        <div className="deepBlue pt-40 bg-gray-100 min-h-screen py-12 px-4 sm:px-6 lg:px-8">
