        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {notices &&
            notices.map((notice) => (
              <div
                key={notice.id}
                className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl transform hover:-translate-y-1 border-t-2 border-blue-400"
              >
                <div className="p-6">
                  <h3 className="text-2xl font-semibold text-[#0E4A84] mb-3 line-clamp-2">
                    {notice.title}
                  </h3>
                ...
                <div className="bg-gray-100 px-6 py-4">
                  <Link
                    to={`notice/:${notice.id}`}
                    className="text-[#0E4A84] font-semibold flex items-center justify-between hover:underline"
                  >
                    자세히 보기
                    <FaChevronRight />
                  </Link>
